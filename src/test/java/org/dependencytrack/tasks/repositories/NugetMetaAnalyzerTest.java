...
class NugetMetaAnalyzerTest {
    ...
    class ErrorHandlingTests {
        ...
        @Test
        void testBaseUrlNotFoundBehaviourWhenSettingRepoUrl() throws Exception {

            // Using Mockito's MockedStatic to force an exception in the HttpClient
            // used in AbstractMetaAnalyzer::processHttpRequest
            try (MockedStatic<HttpClientPool> mockedPool = mockStatic(HttpClientPool.class)) {
                CloseableHttpClient mockClient = mock(CloseableHttpClient.class);
                when(mockClient.execute(any(HttpUriRequest.class)))
                        .thenThrow(new UnknownHostException("no-such-api.nuget.test: nodename nor servname provided, or not known"));

                mockedPool.when(HttpClientPool::getClient).thenReturn(mockClient);

                Assertions.assertDoesNotThrow(() -> {
                    var analyzer = new NugetMetaAnalyzer();
                    analyzer.setRepositoryBaseUrl("https://no-such-api.nuget.test");
                });
            }
        }
